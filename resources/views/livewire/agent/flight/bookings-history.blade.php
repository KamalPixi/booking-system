                ...
                <div class="row">
                    <div class="col px-0">
                        ...
                        <select wire:model="filter_by" class="form-control form-control-sm">
                            <option value="BOOKING_DATE">BookingDate</option>
                            <option value="REF">BookingRef</option>
                            <option value="PNR">BookingPNR</option>
                            <option value="AIR_PNR">AirPNR</option>
                            <option value="STATUS">BookingStatus</option>
                            <option value="PAYMENT_STATUS">PaymentStatus</option>
                        </select>
                    </div>
        ...
        <div>
            ...
            <tbody>
                ...
                <tr class="text-small" wire:key="{{ uniqid() }}">
                    ...
                    <td class="text-center">{{ $booking->createdBy->name }}</td>
                    <td class="text-center">{{ date('dMY', strtotime($booking->created_at)) }}</td>
                    <td class="text-center">{{ $booking->reference }}</td>
                    <td class="text-center"><span class="badge badge-secondary">{{ $booking->confirmation_id }}</span></td>
                    ...
                    <td>
                        <a href="{{ route('b2b.flight.show', ['id' => $booking->reference]) }}" class="btn btn-primary btn-sm px-1 py-0">
                            <small>
                                VIEW
                            </small>
                        </a>
